...
export class FriendController {
  ...
  async addFriend(
    ...
    const [response] = await this.mapToResponseWithDetails([friendship]);
    return response;
  }
    ...
    return Promise.all(
      friendships.map(async (friendship) => {
        ...
        const friendClientId = inverse
          ? friendship.clientId
          : friendship.friendId;
        const friendClient = await this.friendService.getClientById(friendClientId);

        if (friendClient) {
          response.friendId = friendClient.auditoryId;
