...
import { CourseStatsResponseDto } from '../dtos/course-stats-response.dto';
import { CourseMapService } from '@modules/map/application/services/course-map.service';
import { ICourseMapRepository } from '@modules/map/domain/interfaces/course-map.repository.interface';
import { ILessonRepository } from '@modules/lesson/domain/interfaces/lesson.repository.interface';
import { CourseSubscriptionService } from '@modules/profile/application/services/course-subscription.service';
...
export class CourseService {
  ...
  async createCourse(
    ...
    const course = new Course(
      ...
      createCourseDto.fullDescription,
  ...
  async getCourseStats(id: string): Promise<CourseStatsResponseDto> {
    const course = await this.courseRepository.findById(id);
    ...
    const [courseMap, studentCount] = await Promise.all([
      this.courseMapRepository.findByCourseId(id),
      this.courseSubscriptionService.getStudentCountByCourseId(id),
    ]);
