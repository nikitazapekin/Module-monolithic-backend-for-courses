...
export class CourseMapController {
  ...
  @ApiResponse({ status: 201, description: 'Карта курса создана', type: CourseMapResponseDto })
  @ApiResponse({ status: 409, description: 'Карта для этого курса уже существует' })
  async createCourseMap(@Body() createDto: CreateCourseMapDto): Promise<CourseMapResponseDto> {
  ...
  @ApiResponse({ status: 200, description: 'Карта курса', type: CourseMapResponseDto })
  ...
  @ApiResponse({ status: 200, description: 'Карта курса', type: CourseMapResponseDto })
  @ApiResponse({ status: 404, description: 'Карта курса не найдена' })
  async getCourseMapByCourseId(@Param('courseId') courseId: string): Promise<CourseMapResponseDto> {
  ...
  @ApiParam({ name: 'id', description: 'ID карты курса' })
  @ApiResponse({ status: 200, description: 'Карта курса обновлена', type: CourseMapResponseDto })
  @ApiResponse({ status: 404, description: 'Карта курса не найдена' })
  ...
  @ApiResponse({ status: 404, description: 'Карта курса не найдена' })
  async deleteCourseMap(@Param('id') id: string): Promise<{ success: boolean }> {
  ...
  @ApiResponse({ status: 201, description: 'Элемент добавлен', type: MapElementDto })
  @ApiResponse({ status: 404, description: 'Карта курса не найдена' })
  async addMapElement(
  ...
  @ApiResponse({ status: 200, description: 'Элемент карты', type: MapElementDto })
  @ApiResponse({ status: 404, description: 'Элемент карты не найден' })
  async getMapElement(@Param('elementId') elementId: string): Promise<MapElementDto> {
  ...
  @ApiParam({ name: 'mapId', description: 'ID карты курса' })
  @ApiQuery({ name: 'type', required: false, enum: MapElementType, description: 'Фильтр по типу элемента' })
  @ApiResponse({ status: 200, description: 'Список элементов карты', type: [MapElementDto] })
  ...
  @ApiParam({ name: 'elementId', description: 'ID элемента карты' })
  @ApiResponse({ status: 200, description: 'Элемент обновлен', type: MapElementDto })
  @ApiResponse({ status: 404, description: 'Элемент карты не найден' })
