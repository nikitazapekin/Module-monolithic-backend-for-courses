...
export class LessonDetailsController {
    ...
    @Post()
    @ApiOperation({ summary: 'Создать lesson_details для урока (со слайдами и тестами)' })
    @ApiResponse({ status: 201, type: LessonDetailsResponseDto })
    async create(@Body() dto: CreateLessonDetailsDto): Promise<LessonDetailsResponseDto> {
    ...
    async getByLessonId(@Param('lessonId') lessonId: string): Promise<LessonDetailsResponseDto> {
        return this.facade.getLessonDetailsByLessonId(lessonId);
    }

    @Put(':id')
    @ApiOperation({ summary: 'Обновить lesson_details (слайды и тесты заменяются полностью)' })
    @ApiResponse({ status: 200, type: LessonDetailsResponseDto })
    @ApiParam({ name: 'id', type: String })
    async update(
        @Param('id') id: string,
