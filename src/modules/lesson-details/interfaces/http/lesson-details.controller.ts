...
export class LessonDetailsController {
  ...
  @ApiOperation({ summary: 'Получить lesson_details по ID контрольной точки' })
  @ApiResponse({ status: 200, type: LessonDetailsResponseDto })
  @ApiParam({ name: 'checkpointId', type: String })
  async getByCheckpointId(
    @Param('checkpointId') checkpointId: string,
  ): Promise<LessonDetailsResponseDto> {
    return this.facade.getLessonDetailsByCheckpointId(checkpointId);
  }
  ...
  @ApiOperation({ summary: 'Удалить lesson_details по ID контрольной точки' })
  @ApiResponse({
    status: 200,
    schema: { properties: { success: { type: 'boolean' } } },
  })
  @ApiParam({ name: 'checkpointId', type: String })
  async deleteByCheckpointId(
    @Param('checkpointId') checkpointId: string,
  ): Promise<{ success: boolean }> {
    return this.facade.deleteLessonDetailsByCheckpointId(checkpointId);
  }
}
