...
export class LessonDetailsRepository implements ILessonDetailsRepository {
  ...
  async findByCheckpointId(checkpointId: string): Promise<LessonDetails | null> {
    const entity = await this.repo.findOne({ where: { checkpointId } });
    return entity ? this.toDomain(entity) : null;
  }
  ...
  async deleteByCheckpointId(checkpointId: string): Promise<boolean> {
    const result = await this.repo.delete({ checkpointId });
    return result.affected! > 0;
  }

  private toDomain(entity: LessonDetailsOrmEntity): LessonDetails {
    const d = new LessonDetails(entity.lessonId, entity.checkpointId, entity.id);
  ...
  private toOrm(details: LessonDetails): LessonDetailsOrmEntity {
    ...
    entity.lessonId = details.lessonId ?? null;
    entity.checkpointId = details.checkpointId ?? null;
