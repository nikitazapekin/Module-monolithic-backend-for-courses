...
export class LessonDetailsResponseDto {
    ...
    @ApiProperty({ description: 'ID урока' }) @Expose() lessonId: string;
    @ApiProperty({ type: [SlideResponseDto] }) @Expose() @Type(() => SlideResponseDto) slides: SlideResponseDto[];
    @ApiProperty({ type: [TestResponseDto] }) @Expose() @Type(() => TestResponseDto) tests: TestResponseDto[];
    @ApiProperty() @Expose() createdAt: Date;
    @ApiProperty() @Expose() updatedAt: Date;
}
