...
export class CodingTasksController {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Получить все решения задачи' })
  async getTaskSolutions(@Param('taskId') taskId: string) {
    return this.codingTasksService.getTaskSolutions(taskId);
  }

  @Get(':taskId/solutions/language/:language')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Получить решения задачи по языку' })
  async getTaskSolutionsByLanguage(
    @Param('taskId') taskId: string,
    @Param('language') language: string,
  ) {
    return this.codingTasksService.getTaskSolutionsByLanguage(taskId, language);
  }

  @Get(':taskId/statistics')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Получить статистику по задаче' })
  async getTaskStatistics(@Param('taskId') taskId: string) {
    return this.codingTasksService.getTaskStatistics(taskId);
  }

  @Get('solutions/user')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Получить решения пользователя' })
  async getUserSolutions(@Req() req: any) {
    const auditoryId = this.extractAuditoryId(req);
    const client = await this.findClient(auditoryId);
    return this.codingTasksService.getUserSolutions(client.id);
  }

  @Get(':taskId/ranking/:language')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Получить рейтинг по времени выполнения' })
  async getExecutionTimeRanking(
    @Param('taskId') taskId: string,
    @Param('language') language: string,
  ) {
    return this.codingTasksService.getExecutionTimeRanking(taskId, language);
  }

  @Get(':taskId/rank/:language')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Получить ранг пользователя по времени выполнения' })
  async getUserRank(
    @Param('taskId') taskId: string,
    @Param('language') language: string,
    @Req() req: any,
    ...
    const auditoryId = this.extractAuditoryId(req);
    const client = await this.findClient(auditoryId);
    return this.codingTasksService.getUserRank(client.id, taskId, language);
  }

  @Post('solutions/:solutionId/like')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Поставить лайк решению' })
  async likeSolution(
    ...
    const auditoryId = this.extractAuditoryId(req);
    const client = await this.findClient(auditoryId);
    return this.codingTasksService.likeSolution(solutionId, client.id);
  }

  @Post('solutions/:solutionId/dislike')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Поставить дизлайк решению' })
  async dislikeSolution(
    @Param('solutionId') solutionId: string,
    @Req() req: any,
  ) {
    const auditoryId = this.extractAuditoryId(req);
    const client = await this.findClient(auditoryId);
    return this.codingTasksService.dislikeSolution(solutionId, client.id);
  }
