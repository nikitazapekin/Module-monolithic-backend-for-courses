...
export class LessonCommentFacade {
  ...
  deleteComment(id: string, userId: string): Promise<{ success: boolean }> {
  ...
  toggleLike(id: string, userId: string): Promise<LessonCommentResponseDto> {
    return this.service.toggleLike(id, userId);
  }

  toggleDislike(id: string, userId: string): Promise<LessonCommentResponseDto> {
